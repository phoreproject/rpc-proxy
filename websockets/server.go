...
func Run(hub *Hub, client *rpcclient.Client) *http.ServeMux {
	...
	go hub.Run()
	rpcServer.HandleFunc("/notifyBlock", func(w http.ResponseWriter, r *http.Request) {
		r.ParseForm()
		for k := range r.Form {
	...
	rpcServer.HandleFunc("/notifyMempool", func(w http.ResponseWriter, r *http.Request) {
		r.ParseForm()
		for k := range r.Form {
			NotificationMempoolHandler(hub, client, k)
			return
		}
	})
	return rpcServer
